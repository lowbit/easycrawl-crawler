...
async function processJob(crawlerConfig) {
  try {
    console.log(`Starting job ${crawlerConfig.job_id} for website ${crawlerConfig.website_code}`);
    ...
    if (items != null && items.length > 0) {
      await insertItems(crawlerConfig.code, items, crawlerConfig.job_id, crawlerConfig.website_code);
    ...
    await insertCrawlError(crawlerConfig.job_id, crawlerConfig.website_code, crawlerConfig.code, error);
  } finally {
...
async function checkAndProcessJobs() {
  try {
    if (runningWebsites.size < MAX_CONCURRENT_JOBS) {
      ...
      while ((websiteCrawlerConfig = await processCrawlJobs()) !== null &&
...
async function main() {
  ...
  if (!isConnected) {
    console.error('Failed to establish initial database connection. Exiting...');
    process.exit(1);
  }
